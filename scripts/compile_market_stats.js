/**
 * Script d'agrégation et de calcul des statistiques de marché Leboncoin
 * Détermine le prix et kilométrage moyen/médian le plus facilement disponible sur le marché de l'occasion
 * 
 * Exécution : node scripts/compile_market_stats.js
 */
...
const adsSourcePath = path.resolve(__dirname, '../src/data/rawHarvestedAds.json');
const outputPath = path.resolve(__dirname, '../src/data/marketPriceStats.json');
...
function isQuadricycle(title) {
  return QUADRICYCLE_REGEX.test(title);
}

function matchAdToModelId(adTitle, adYear) {
  if (isQuadricycle(adTitle)) {
...
function run() {
  if (!fs.existsSync(adsSourcePath)) {
    console.error('File not found:', adsSourcePath);
    process.exit(1);
  }

  const raw = fs.readFileSync(adsSourcePath, 'utf8');
  const ads = JSON.parse(raw);
  console.log(`Processing ${ads.length} scraped ads from Leboncoin (source: ${adsSourcePath})...`);
  ...
  for (const [modelId, modelAds] of Object.entries(grouped)) {
    const validPrices = modelAds
      .map((a) => a.price)
      .filter((p) => typeof p === 'number' && p >= 3000)
      .sort((a, b) => a - b);
  ...
  console.log(`Compilation complete: saved statistics for ${Object.keys(results).length} models into ${outputPath}`);
}
