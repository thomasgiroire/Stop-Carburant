...
export const DEPARTMENT_MAP = new Map<string, Department>(
  DEPARTMENTS.map((d) => [d.code, d])
);

export function getDepartmentByCode(code: string): Department | undefined {
  const normalized = code.trim().toUpperCase().padStart(2, '0');
  return DEPARTMENT_MAP.get(normalized) || DEPARTMENT_MAP.get(code.trim().toUpperCase());
}
